...
public class ResultWriter {
	...
	private void addContent(Document document) throws DocumentException {
		 ...
		 */
		try {
			Image image = Image.getInstance("aperture 1.png");
	...
	private PdfPTable getResultsTable() {
		...
		for (String age : ageGroups) {
			if (operations.searchAgeGroupInResult(results, age)) {
				// add to the table for each age group that follows
				// Age-group
				...
				for (Result res : results) {
					if (res.getSwimmer().getAgeGroup().equals(age)) {
						// add the swimmer
						PdfPCell c21 = new PdfPCell(new Phrase(Integer.toString(classification), normalFont));
						...
						disableBorders(c25);
						table.addCell(c25);
						PdfPCell c26 = new PdfPCell(new Phrase("FINA", normalFont));
						disableBorders(c26);
						table.addCell(c26);
						PdfPCell c27 = new PdfPCell(new Phrase("Lo", normalFont));
						disableBorders(c27);
						table.addCell(c27);
						classification++;						
					}
